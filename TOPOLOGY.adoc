...
lustreiser generates verified real-time embedded code via Lustre. Lustre
(Caspi and Halbwachs, Grenoble) is a synchronous dataflow language used
in safety-critical domains including avionics (SCADE), nuclear control,
and automotive systems. lustreiser reads control logic descriptions from
a `+lustreiser.toml+` manifest and generates deterministic,
bounded-execution-time C code via Lustre intermediate representation. It
targets embedded systems engineers who need provably correct, race-free
real-time control logic without writing Lustre directly.
